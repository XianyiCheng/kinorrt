...
class RRTManipulation(object):
    ...
    def best_mnp_location(self, tree, x_near, x_rand, mode, vel):
        ...
        if mnps is not None:
            ...
            #preprocess = self.smsolver.preprocess(x_near, self.env_mu, self.mnp_mu, envs, mnps, e_modes, self.h_modes, self.object_weight, self.mnp_fn_max)
            #d_proj = velocity_project_direction(v_star, mnps, envs, mode)
            #d_proj_ = velocity_project_direction(v_star_, mnps, envs, mode)
            #score = self.smsolver.stability_margin(preprocess, vel, mode)
