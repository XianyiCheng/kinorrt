...
def test_kinorrt_cases(stability_solver, max_samples = 100):
    ...
    X_dimensions = np.array([(-1.5, 1.5), (-1.5, 1.5), (-1.5*np.pi, 1.5*np.pi)])
    x_init = (0,0,0)
    x_goal = (0,0,np.pi/2)
    world_key = 'vert'
    dist_weight = 1
    manipulator = doublepoint_manipulator(np.array([[-1.5,-1.5,0.2,-1.5],[-0.2,1.5,1.5,1.5]]))
    mnp_fn_max = 100
    goal_kch = [0.1, 0.1, 1]
...
for i in [1]:
    ...
    ti = test_kinorrt_cases(stability_solver, max_samples=200)
